...
export const createExpense = async (req, res) => {
  ...
  if (io) io.to(String(shopId)).emit("expense:new", { expenseId: exp._id, amount: exp.amount, category: exp.category, createdAt: exp.createdAt });
  res.json(exp);
};
export const getExpenses = async (req, res) => {
  ...
  const data = await Expense.find(q).sort({ createdAt: -1 }).skip((Number(page) - 1) * Number(limit)).limit(Number(limit));
