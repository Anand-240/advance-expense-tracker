...
export const createSimulatedPayment = async (req, res) => {
  const { shopId, amount, method = "UPI", customer } = req.body;
  ...
  await Shop.updateOne(
    { _id: shopId },
    { $inc: { totalRevenue: amount, totalSales: 1 } }
  );
  if (customer?.name) {
    const up = await Customer.findOneAndUpdate(
      { shopId, phone: customer.phone || null, email: customer.email || null, name: customer.name },
      { $inc: { totalSpent: amount, totalVisits: 1 }, $set: { lastPurchaseAt: new Date() } },
      { upsert: true, new: true }
    );
    if (!payment.customerId) {
      payment.customerId = up._id;
      await payment.save();
    }
  }
  io.to(String(shopId)).emit("payment:new", {
...
export const refundPayment = async (req, res) => {
  ...
  io.to(String(pay.shopId)).emit("payment:update", { paymentId: pay._id, status: "refunded" });
  res.json(pay);
};