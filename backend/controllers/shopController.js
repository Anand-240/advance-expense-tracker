...
export const createShop = async (req, res) => {
  ...
  const shop = await Shop.create({ ownerId: req.user._id, name, ownerName, email, phone, address });
  res.json(shop);
};
...
export const getShopSummary = async (req, res) => {
  ...
  const salesAgg = await Payment.aggregate([{ $match: match }, { $group: { _id: null, revenue: { $sum: "$amount" }, sales: { $sum: 1 } } }]);
  ...
  const expAgg = await Expense.aggregate([{ $match: expMatch }, { $group: { _id: null, expenses: { $sum: "$amount" } } }]);
